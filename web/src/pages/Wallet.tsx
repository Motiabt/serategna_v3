...
export function Wallet() {
  ...
  async function saveSavings() {
    ...
    setSavings(g); setSavSheet(false); toast.success(t('savingsGoalSet'));
  }
  ...
  async function withdraw() {
    ...
    try {
      ...
      setError(e instanceof ApiError ? e.message : t('failedWord'));
    } finally {
      setBusy(false);
    }
  }
  ...
  if (data.error)
    return <PageHeader title={t('wallet')} sub={t('switchToWorkWallet')} />;

  return (
    <div className="h-full overflow-y-auto pb-6 no-scrollbar">
      <PageHeader title={t('verifiedIncome')} sub={t('paidDirectlyProof')} />

      <div className="px-5 pt-2">
        <div className="hero overflow-hidden p-5">
          <p className="text-xs text-white/70">{t('verifiedIncomeOn')}</p>
          ...
          <div className="mt-1 flex items-center gap-3 text-xs text-white/70">
            <span>{score?.jobsCompleted ?? 0} {t('paidJobs')}</span>
            <span>·</span>
          ...
          <div className="mt-4 flex items-start gap-2 rounded-2xl bg-white/10 px-3 py-2 text-xs text-white/90">
            ...
            <span>{t('paidDirectlyNotePre')} <b>{t('directlyWord')}</b> {t('paidDirectlyNotePost')}</span>
          </div>
          {(score?.jobsCompleted ?? 0) === 0 && (
            <button onClick={() => nav('/app/jobs')} className="mt-4 w-full rounded-2xl bg-white py-3 text-sm font-semibold text-brand-700">{t('findWorkToEarn')}</button>
          )}
        </div>
      </div>
      ...
      <div className="px-5 pt-5">
        <div className="card p-4">
          <div className="flex items-center justify-between">
            <h2 className="flex items-center gap-1.5 text-base font-bold text-ink"><PiggyBank className="h-4 w-4 text-brand-600" /> {t('iqubSavings')}</h2>
            <button onClick={() => { setSavForm({ target: String(savings?.targetAmount ?? ''), rate: String(savings?.ratePct ?? 10) }); setSavSheet(true); }} className="flex items-center gap-1 text-xs font-semibold text-brand-700"><Pencil className="h-3 w-3" /> {savings ? t('edit') : t('setGoalWord')}</button>
          </div>
          {savings && savings.targetAmount > 0 ? (
            <>
              <div className="mt-2 flex items-end justify-between">
                <p className="text-2xl font-extrabold text-ink">{etb(savings.savedAmount)}</p>
                <p className="text-xs text-muted">{t('ofWord')} {etb(savings.targetAmount)}</p>
              </div>
              ...
              <p className="mt-2 text-xs text-muted">{savings.ratePct}% {t('iqubRateNote')}</p>
            </>
          ) : (
            <p className="mt-1 text-sm text-muted">{t('iqubSetGoalNote')}</p>
          )}
        </div>
      </div>

      <div className="px-5 pt-6">
        <h2 className="mb-3 text-base font-bold text-ink">{t('recentEarnings')}</h2>
        <div className="space-y-2">
          {(data.earnings ?? []).length === 0 && (
            <p className="rounded-2xl bg-white p-4 text-sm text-muted shadow-card">{t('noEarningsYet')}</p>
          )}
      ...
      {(data.payouts ?? []).length > 0 && (
        <div className="px-5 pt-6">
          <h2 className="mb-3 text-base font-bold text-ink">{t('withdrawalsTitle')}</h2>
          <div className="space-y-2">
      ...
      <Sheet open={sheet} onClose={() => setSheet(false)} title={t('withdrawEarnings')}>
        <label className="label">{t('amountWord')} ({t('maxWord')} {etb(data.withdrawable)})</label>
        <input className="input" inputMode="numeric" value={amount} onChange={(e) => setAmount(e.target.value.replace(/\D/g, ''))} placeholder="0" />
        <label className="label mt-4">{t('destinationWord')}</label>
        ...
        <p className="mt-2 text-center text-xs text-muted">{t('sentToOwnPre')} {dest} {t('sentToOwnPost')}</p>
      </Sheet>

      <Sheet open={savSheet} onClose={() => setSavSheet(false)} title={t('iqubGoalTitle')}>
        <label className="label">{t('goalAmountEtb')}</label>
        ...
        <label className="label mt-4">{t('setAsidePerPayment')}</label>
        ...
        <button onClick={saveSavings} disabled={!savForm.target} className="btn-brand mt-5 w-full"><PiggyBank className="h-4 w-4" /> {t('saveGoal')}</button>
      </Sheet>
    </div>
  );
}
